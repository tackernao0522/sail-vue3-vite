## 116. 外部結合し0も表示できるように対応

### 動作検証

```
Apiじゃない方のAnalysisControllerに
rfm処理部分を再度コピペ
...
class AnalysisController extends Controller
{
    ...
    {
        // 期間指定(短くしてみる)
        $startDate = '2022-08-20'; // 編集
        $endDate = '2022-8-21'; // 編集
...
+ [LEFT JOIN／RIGHT JOIN句](https://readouble.com/laravel/9.x/ja/queries.html) <br>
...
class AnalysisController extends Controller
{
    public function index()
    {
        ...
        $rCount = DB::table($subQuery)
            ->rightJoin('ranks', 'ranks.rank', '=', 'r'); // 追加
            // 一旦コメントアウト 確認したら解除する
            // ->groupBy('r')
            // ->selectRaw('r, count(r)')
            // ->orderBy('r', 'desc')
            // ->pluck('count(r)');
            dd($rCount->get()); // 追加 確認後コメントアウト
...
class AnalysisController extends Controller
{
    public function index()
    {
        ...
        dd($total, $eachCount, $rCount, $fCount, $mCount); // 確認後コメントアウト
        // ここまで
...
class RFMService
{
  public static function rfm($subQuery, $rfmPrms)
  {
    ...
    $rCount = DB::table($subQuery)
      ->rightJoin('ranks', 'ranks.rank', '=', 'r') // 編集
      ->groupBy('rank') // 編集
      ->selectRaw('rank as r, count(r)') // 編集
      ->orderBy('r', 'desc')
      ->pluck('count(r)');
    ...
    $fCount = DB::table($subQuery)
      ->rightJoin('ranks', 'ranks.rank', '=', 'f') // 編集
      ->groupBy('rank') // 編集
      ->selectRaw('rank as f, count(f)') // 編集
      ->pluck('count(f)');

    $mCount = DB::table($subQuery)
      ->rightJoin('ranks', 'ranks.rank', '=', 'm') // 編集
      ->groupBy('rank') // 編集
      ->selectRaw('rank as m, count(m)') // 編集
      ->orderBy('m', 'desc')
      ->pluck('count(m)');
    ...
    $data = DB::table($subQuery)
      ->rightJoin('ranks', 'ranks.rank', '=', 'f') // 編集
      ->groupBy('rank') // 編集
      ->selectRaw('concat("r_", rank) as rRank, // 編集
            count(case when f = 5 then 1 end ) as f_5,
            count(case when f = 4 then 1 end ) as f_4,
            count(case when f = 3 then 1 end ) as f_3,
            count(case when f = 2 then 1 end ) as f_2,
